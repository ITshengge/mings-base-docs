...
| heartBeatDuration | 发送心跳间隔时长，默认每隔 5 秒一次，如果设置为 0，那么表示不需要发心跳检测 | number  | 5000                                                                     | 否       |
| reconnectCount    | 重连次数，默认 5 次，如果为 0，那么表示不限次数，一直会重连，直到成功为止   | number  | 5                                                                        | 否       |
| needHeartBeat     | 是否需要发送心跳检测，默认不需要发送心跳检测                                | boolean | false                                                                    | 否       |
